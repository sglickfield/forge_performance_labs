...
import { rateAthlete, setDraft, setShareToken, signAthlete, unlockAthlete } from './store.ts'
...
describe('coach rating', () => {
  it('stores a verdict on the letter without requiring a signature', () => {
    ...
    const record = next.athletes['FPL-0001']
  ...
  it('lets them sign without a rating, and rate after lock', () => {
    ...
    expect(signed.athletes['FPL-0001']?.status).toBe('signed')
    expect(signed.athletes['FPL-0001']?.coachRating).toBeUndefined()
    const rated = rateAthlete(signed, 'FPL-0001', 'ready')
    expect(rated.athletes['FPL-0001']?.status).toBe('signed')
    expect(rated.athletes['FPL-0001']?.coachRating?.verdict).toBe('ready')
  })

  it('can stamp a verdict at sign time', () => {
    const signed = signAthlete(sessionWithLetter(), 'FPL-0001', 'rewrite')
    expect(signed.athletes['FPL-0001']?.coachRating?.verdict).toBe('rewrite')
    expect(signed.athletes['FPL-0001']?.signedBy).toBe('Alex F')
  })
  ...
  it('clears the rating on unlock and on a new draft', () => {
    ...
    expect(unlocked.athletes['FPL-0001']?.status).toBe('draft')
    expect(unlocked.athletes['FPL-0001']?.coachRating).toBeUndefined()

    const again = rateAthlete(unlocked, 'FPL-0001', 'edited')
    const record = again.athletes['FPL-0001']
    ...
    expect(regenerated.athletes['FPL-0001']?.coachRating).toBeUndefined()
    expect(regenerated.athletes['FPL-0001']?.status).toBe('draft')
  })

  it('clears a share token on unlock', () => {
    const signed = signAthlete(sessionWithLetter(), 'FPL-0001')
    const shared = setShareToken(signed, 'FPL-0001', 'abcdefghijklmnopqr')
    expect(shared.athletes['FPL-0001']?.shareToken).toBe('abcdefghijklmnopqr')
    const unlocked = unlockAthlete(shared, 'FPL-0001')
    expect(unlocked.athletes['FPL-0001']?.shareToken).toBeUndefined()
  })

  it('does not rate a sheet with no letter', () => {
    const empty = sessionWithLetter()
    const id = 'FPL-0001'
    const record = empty.athletes[id]!
    empty.athletes[id] = { ...record, letter: undefined, draft: undefined, status: 'new' }
    expect(rateAthlete(empty, id, 'ready').athletes[id]?.coachRating).toBeUndefined()
  })
})
